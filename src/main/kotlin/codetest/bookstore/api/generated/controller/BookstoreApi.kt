...
interface BookstoreApi {
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(
                responseCode = "200",
                description = "取得成功",
                content = [Content(schema = Schema(implementation = BookAuthorSet::class))]
            ),
            ApiResponse(
                responseCode = "400",
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(
                responseCode = "200",
                description = "取得成功",
                content = [Content(array = ArraySchema(schema = Schema(implementation = AuthorAndBookListInner::class)))]
            ),
            ApiResponse(
                responseCode = "404",
                description = "Not found",
    ...
    fun bookstoreGet(
        @Parameter(description = "著者を指定します。指定がない場合は登録されている情報すべてを取得します。") @RequestParam(
            value = "authorName",
            required = false
        ) authorName: String?
    ): ResponseEntity<List<AuthorAndBookListInner>> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }
    ...
    fun bookstorePost(
        @Parameter(
            description = "",
            required = true
        ) @RequestBody bookAndAuthorRequest: BookAndAuthorRequest
    ): ResponseEntity<BookAuthorSet> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }
}
