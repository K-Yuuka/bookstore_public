...
import com.quodigital.recruit.codetest.bookstore.db.generated.indexes.BOOK_IX1
import com.quodigital.recruit.codetest.bookstore.db.generated.indexes.BOOK_IX2
import com.quodigital.recruit.codetest.bookstore.db.generated.keys.BOOK_AUTHOR__BOOK_AUTHOR_FK2
import com.quodigital.recruit.codetest.bookstore.db.generated.keys.BOOK_PKC
import com.quodigital.recruit.codetest.bookstore.db.generated.tables.JAuthor.JAuthorPath
import com.quodigital.recruit.codetest.bookstore.db.generated.tables.JBookAuthor.JBookAuthorPath
...
open class JBook(
     ...
     */
    val BOOK_ID: TableField<JBookRecord, Int?> = createField(DSL.name("book_id"), SQLDataType.INTEGER.nullable(false).identity(true), this, "")

    /**
     * The column <code>public.book.book_name</code>.
     */
    val BOOK_NAME: TableField<JBookRecord, String?> = createField(DSL.name("book_name"), SQLDataType.VARCHAR(255).nullable(false), this, "")

    private constructor(alias: Name, aliased: Table<JBookRecord>?): this(alias, null, null, null, aliased, null, null)
    private constructor(alias: Name, aliased: Table<JBookRecord>?, parameters: Array<Field<*>?>?): this(alias, null, null, null, aliased, parameters, null)
    private constructor(alias: Name, aliased: Table<JBookRecord>?, where: Condition?): this(alias, null, null, null, aliased, null, where)
    ...
    constructor(path: Table<out Record>, childPath: ForeignKey<out Record, JBookRecord>?, parentPath: InverseForeignKey<out Record, JBookRecord>?): this(Internal.createPathAlias(path, childPath, parentPath), path, childPath, parentPath, BOOK, null, null)

    /**
     * A subtype implementing {@link Path} for simplified path-based joins.
     */
    open class JBookPath : JBook, Path<JBookRecord> {
        constructor(path: Table<out Record>, childPath: ForeignKey<out Record, JBookRecord>?, parentPath: InverseForeignKey<out Record, JBookRecord>?): super(path, childPath, parentPath)
     ...
     * Create an inline derived table from this table
     */
    @PlainSQL override fun where(condition: SQL): JBook = where(DSL.condition(condition))

    /**
     * Create an inline derived table from this table
     */
    @PlainSQL override fun where(@Stringly.SQL condition: String): JBook = where(DSL.condition(condition))

    /**
     * Create an inline derived table from this table
     */
    @PlainSQL override fun where(@Stringly.SQL condition: String, vararg binds: Any?): JBook = where(DSL.condition(condition, *binds))

    /**
     * Create an inline derived table from this table
     */
    @PlainSQL override fun where(@Stringly.SQL condition: String, vararg parts: QueryPart): JBook = where(DSL.condition(condition, *parts))
