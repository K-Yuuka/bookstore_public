...
@WebMvcTest
class BookstoreApiImplTest(@Autowired val mockMvc: MockMvc) {
    @MockkBean
    ...
    fun bookstorePost_successful() {
        ...
        every { bookStoreService.addBookAndAuthor(any(), any()) } returns Result.success(expected)
    ...
    fun bookstorePost_failure_conflict() {
        ...
        every { bookStoreService.addBookAndAuthor(any(), any()) } returns Result.failure(
            ConflictException(
                exceptionMessage
            )
        )
    ...
    fun bookstoreGet_successful_emptyName() {
        every { bookStoreService.getBookListByAuthorName(null) } returns Result.success(
            TestUtils.buildAuthorAndRelationalBooksList("BookAuthorRepositoryProvider_3.json")
        )
        every { bookStoreService.getBookListByAuthorName("name1") } returns Result.success(
    ...
    fun bookstoreGet_failure_notFound() {
        val exceptionMessage = "test exception"
        every { bookStoreService.getBookListByAuthorName(any()) } returns Result.failure(
            NotFoundException(
                exceptionMessage
            )
        )
    ...
    fun bookstoreBookIdPut_successful() {
        ...
        every { bookStoreService.editAuthor(any(), any()) } returns Result.success(expectedValue)
    ...
    fun bookstoreBookIdPut_failure_notFound() {
        val exceptionMessage = "test exception"
        every { bookStoreService.editAuthor(any(), any()) } returns Result.failure(NotFoundException(exceptionMessage))
        val requestJson = TestUtils.objectToJsonString(AuthorRequest("name1"))
        val result = this.mockMvc
            .perform(
                MockMvcRequestBuilders.put("/bookstore/1")
    ...
    fun bookstoreBookIdPut_failure_conflict() {
        val exceptionMessage = "test exception"
        every { bookStoreService.editAuthor(any(), any()) } returns Result.failure(ConflictException(exceptionMessage))
        val requestJson = TestUtils.objectToJsonString(AuthorRequest("name1"))
        val result = this.mockMvc
            .perform(
                MockMvcRequestBuilders.put("/bookstore/1")
    ...
    fun bookstoreBookIdPut_failure_internalServerError() {
        ...
        } returns Result.failure(NullPointerException(exceptionMessage))
        val requestJson = TestUtils.objectToJsonString(AuthorRequest("name1"))
        val result = this.mockMvc
            .perform(
                MockMvcRequestBuilders.put("/bookstore/1")
